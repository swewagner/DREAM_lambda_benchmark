    ...
    for (auto const & [sequence, id, quality] : q_fin)
    {
        ...
        else if (insert_mode < 9)
        {
            ...
            std::vector<seqan3::aa27> seq_mid = bins[bin_id][0].sequence() |
                                                seqan3::views::slice(insert_start_pos + match_len, insert_start_pos + insert_start_pos) |
                                                seqan3::ranges::to<std::vector>();
            std::vector<seqan3::aa27> seq_suf = bins[bin_id][0].sequence() |
                                                seqan3::views::slice(insert_start_pos + insert_start_pos + match_len, std::ranges::size(bins[bin_id][0].sequence())) | seqan3::ranges::to<std::vector>();
