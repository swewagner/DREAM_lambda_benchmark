...
match_info insert_match(std::vector<std::vector<sequence_record_type>> & bins, std::vector<seqan3::dna4> match, uint32_t & bin_id, uint32_t seq_idx, u_int32_t match_len, std::string query_id, uint64_t query_match_start_pos)
{
...
void run_program(cmd_arguments const & args)
{
    ...
    // IO
    //record input type: seqan3::sequence_file_input<dna4_input_trait>::record_type
    seqan3::sequence_file_input<dna4_input_trait> q_fin{args.query_file_path};
    seqan3::sequence_file_input<dna4_input_trait> r_fin{args.ref_file_path};
    ...
    for (auto const & [sequence, id, quality] : q_fin)
    {
        ...
        std::uniform_int_distribution<uint64_t> match_start_dis(0, query_length - args.match_len);
        ...
        {
            //insert to 1 bin
            match_info mi = insert_match(bins, match, nums[idx], 0, args.match_len, id, match_start_pos);
        ...
        else if (insert_mode < 8)
        {
            ...
            match_info mi = insert_match(bins, match, nums[idx], 0, args.match_len, id, match_start_pos);
            match_infos.push_back(mi);
            match_info mi2 = insert_match(bins, match, nums[idx], 1, args.match_len, id, match_start_pos);
        ...
        else if (insert_mode == 9)
        {
            ...
            match_infos.push_back(mi);
            ++idx;
            match_info mi2 = insert_match(bins, match, nums[idx], 0, args.match_len, id, match_start_pos);
            match_infos.push_back(mi2);
        }
    }
